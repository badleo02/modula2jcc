...
public class SlkToken {
    ...
    public short get() {
        ...
        // Recuperamos el tipo de Token
        TipoToken tipo = _token.getTipoToken();

       // insertarNodo(tipo);

        if((tipo.equals(TipoToken.PALABRA_RESERVADA))){
            
            if(_token.getAtributo().equals("BOOLEAN") || 
                    _token.getAtributo().equals("TRUE") || _token.getAtributo().equals("FALSE"))
                insertarNodo(tipo, _token.getAtributo());
        }
        
         if (tipo.equals(TipoToken.NUMERO_REAL)) {

             if (_pilaNodos.esParteDeclarativa())
                _gestorDeErrores.insertaErrorSemantico(new TErrorSemantico(ErroresSemanticos.ERROR_SEMANTICO_ENTERO_MAL_COLOCADO,
                        "Numero real mal colocado",
                        _token._linea,
                        _token._columna));
                //_logger.error("Numero real mal colocado");
            else
                insertarNodo(tipo, _token.getAtributo());
        }

        if (tipo.equals(TipoToken.IDENTIFICADOR)) {        
                insertarNodo(tipo, _token.getAtributo());
            
        }

        if (tipo.equals(TipoToken.PALABRA_RESERVADA) && _token.getAtributo().equals("INTEGER"))
            insertarNodo(tipo, _token.getAtributo());
        
        if (tipo.equals(TipoToken.PALABRA_RESERVADA) && _token.getAtributo().equals("REAL"))          
            insertarNodo(tipo, _token.getAtributo());
        
        if (tipo.equals(TipoToken.NUMERO_ENTERO)) {

            if (_pilaNodos.esParteDeclarativa())
                _gestorDeErrores.insertaErrorSemantico(new TErrorSemantico(ErroresSemanticos.ERROR_SEMANTICO_ENTERO_MAL_COLOCADO,
                        "Numero entero mal colocado",
                        _token._linea,
                        _token._columna));
                //_logger.error("Numero entero mal colocado");
            else 
                insertarNodo(tipo, _token.getAtributo());    
        }
    ...
    private void insertarNodo(TipoToken tipo, String atributo) {
        Nodo nodo = new Nodo();
        nodo.concatRight(tipo);
        nodo.setValor(atributo);
        nodo.setColumna(_token._columna);
        nodo.setLinea(_token._linea);
